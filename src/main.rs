...
async fn main() -> Result<()> {
    ...
    let result = match &cli.command {
        cli::Commands::Query(args) => {
            commands::query::execute(&ctx, args).await
        },
        cli::Commands::ListDatabases(_) => {
            commands::database::list(&ctx).await
        },
        cli::Commands::ListWorkgroups(args) => {
            commands::workgroup::list(&ctx, args).await
        },
        cli::Commands::History(args) => {
            commands::history::list(&ctx, args).await
        },
        cli::Commands::Inspect(args) => {
            commands::inspect::inspect(&ctx, args).await
        },
        cli::Commands::Download(args) => {
            commands::inspect::download(&ctx, args).await
        },
    };
